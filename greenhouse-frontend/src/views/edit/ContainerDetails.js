...
class ContainerDetails extends React.Component{
    ...
    async componentDidMount(){
        const container = await api.getContainerById(this.props.id);
        this.setState(
        {
            devices: container.data.data.devices,
            sensors: container.data.data.sensors,
            name: container.data.data.name,
            address: container.data.data.address,
            channel: container.data.data.channel,
        }
        )
    }
    ...
    save = async () => {
        ...
        if (name !== '') {
            ...
            this.setState({err:''})
            await api.updateContainerById(this.props.id, data).then(res => {
                window.alert('Контейнер обновлен успешно!')
                this.setState({
                  devices: [],
                  sensors: [],
                  name: '',
